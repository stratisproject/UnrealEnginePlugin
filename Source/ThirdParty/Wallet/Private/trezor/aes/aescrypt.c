...
{   uint32_t         locals(b0, b1);
	...
	if(cx->inf.b[0] != 10 * TREZOR_AES_BLOCK_SIZE && cx->inf.b[0] != 12 * TREZOR_AES_BLOCK_SIZE && cx->inf.b[0] != 14 * TREZOR_AES_BLOCK_SIZE)
		return EXIT_FAILURE;
    ...
    switch(cx->inf.b[0])
    {
    case 14 * TREZOR_AES_BLOCK_SIZE:
        round(fwd_rnd,  b1, b0, kp + 1 * N_COLS);
        round(fwd_rnd,  b0, b1, kp + 2 * N_COLS);
        kp += 2 * N_COLS;
        //-fallthrough
    case 12 * TREZOR_AES_BLOCK_SIZE:
        round(fwd_rnd,  b1, b0, kp + 1 * N_COLS);
        round(fwd_rnd,  b0, b1, kp + 2 * N_COLS);
        kp += 2 * N_COLS;
        //-fallthrough
    case 10 * TREZOR_AES_BLOCK_SIZE:
        round(fwd_rnd,  b1, b0, kp + 1 * N_COLS);
        round(fwd_rnd,  b0, b1, kp + 2 * N_COLS);
