...
{   int nb = len >> TREZOR_AES_BLOCK_SIZE_P2;

    if(len & (TREZOR_AES_BLOCK_SIZE - 1))
        return EXIT_FAILURE;
    ...
    {   uint8_t *ksp = (uint8_t*)(ctx->ks);
        ...
        {   aligned_auto(uint8_t, buf, BFR_BLOCKS * TREZOR_AES_BLOCK_SIZE, 16);
            ...
            while(nb)
            {
                ...
                if(ip != ibuf)
                    memcpy(buf, ibuf, m * TREZOR_AES_BLOCK_SIZE);

                via_ecb_op5(ksp, cwd, ip, op, m);

                if(op != obuf)
                    memcpy(obuf, buf, m * TREZOR_AES_BLOCK_SIZE);

                ibuf += m * TREZOR_AES_BLOCK_SIZE;
                obuf += m * TREZOR_AES_BLOCK_SIZE;
                nb -= m;
            }
        }
    ...
    while(nb--)
    {
        if(aes_encrypt(ibuf, obuf, ctx) != EXIT_SUCCESS)
            return EXIT_FAILURE;
        ibuf += TREZOR_AES_BLOCK_SIZE;
        obuf += TREZOR_AES_BLOCK_SIZE;
    }
#endif
    return EXIT_SUCCESS;
}
...
{   int nb = len >> TREZOR_AES_BLOCK_SIZE_P2;
    ...
    {   uint8_t *ksp = (uint8_t*)(ctx->ks), *ivp = iv;
        aligned_auto(uint8_t, liv, TREZOR_AES_BLOCK_SIZE, 16);
        via_cwd(cwd, hybrid, enc, 2 * ctx->inf.b[0] - 192);
        ...
        {
            ivp = liv;
            memcpy(liv, iv, TREZOR_AES_BLOCK_SIZE);
        }
        ...
        if(iv != ivp)
            memcpy(iv, ivp, TREZOR_AES_BLOCK_SIZE);
    ...
    if(!ALIGN_OFFSET( ibuf, 4 ) && !ALIGN_OFFSET( iv, 4 ))
        while(nb--)
        {
            ...
            if(aes_encrypt(iv, iv, ctx) != EXIT_SUCCESS)
                return EXIT_FAILURE;
            memcpy(obuf, iv, TREZOR_AES_BLOCK_SIZE);
            ibuf += TREZOR_AES_BLOCK_SIZE;
            obuf += TREZOR_AES_BLOCK_SIZE;
        }
    else
...
{   unsigned char tmp[TREZOR_AES_BLOCK_SIZE];
    int nb = len >> TREZOR_AES_BLOCK_SIZE_P2;
...
# ifdef FAST_BUFFER_OPERATIONS
    if(!ALIGN_OFFSET( obuf, 4 ) && !ALIGN_OFFSET( iv, 4 ))
        while(nb--)
        {
            memcpy(tmp, ibuf, TREZOR_AES_BLOCK_SIZE);
            if(aes_decrypt(ibuf, obuf, ctx) != EXIT_SUCCESS)
                return EXIT_FAILURE;
            ...
            memcpy(iv, tmp, TREZOR_AES_BLOCK_SIZE);
            ibuf += TREZOR_AES_BLOCK_SIZE;
            obuf += TREZOR_AES_BLOCK_SIZE;
        }
    else
...
{   int cnt = 0, b_pos = (int)ctx->inf.b[2], nb;
    ...
    {
        while(b_pos < TREZOR_AES_BLOCK_SIZE && cnt < len)
        {
            *obuf++ = (iv[b_pos++] ^= *ibuf++);
            cnt++;
        }

        b_pos = (b_pos == TREZOR_AES_BLOCK_SIZE ? 0 : b_pos);
    }

    if((nb = (len - cnt) >> TREZOR_AES_BLOCK_SIZE_P2) != 0)    /* process whole blocks */
    {
#if defined( USE_VIA_ACE_IF_PRESENT )
        ...
        {   int m;
            ...
            if(!ALIGN_OFFSET( ibuf, 16 ) && !ALIGN_OFFSET( obuf, 16 ))
            {
                ...
                ibuf += nb * TREZOR_AES_BLOCK_SIZE;
                obuf += nb * TREZOR_AES_BLOCK_SIZE;
                cnt  += nb * TREZOR_AES_BLOCK_SIZE;
            }
...
# ifdef FAST_BUFFER_OPERATIONS
        if(!ALIGN_OFFSET( ibuf, 4 ) && !ALIGN_OFFSET( obuf, 4 ) && !ALIGN_OFFSET( iv, 4 ))
            while(cnt + TREZOR_AES_BLOCK_SIZE <= len)
            {
                assert(b_pos == 0);
                if(aes_encrypt(iv, iv, ctx) != EXIT_SUCCESS)
                    return EXIT_FAILURE;
...
# endif
    ...
    while(cnt < len)
    {
        ...
        while(cnt < len && b_pos < TREZOR_AES_BLOCK_SIZE)
        {
            *obuf++ = (iv[b_pos++] ^= *ibuf++);
            cnt++;
        }
...
#define BFR_LENGTH  (BFR_BLOCKS * TREZOR_AES_BLOCK_SIZE)
...
{   unsigned char   *ip;
    ...
    if(b_pos)
    {
        memcpy(buf, cbuf, TREZOR_AES_BLOCK_SIZE);
        if(aes_ecb_encrypt(buf, buf, TREZOR_AES_BLOCK_SIZE, ctx) != EXIT_SUCCESS)
            return EXIT_FAILURE;

        while(b_pos < TREZOR_AES_BLOCK_SIZE && len)
        {
            *obuf++ = *ibuf++ ^ buf[b_pos++];
            --len;
        }
    ...
    while(len)
    {
        ...
        for(i = 0, ip = buf; i < (blen >> TREZOR_AES_BLOCK_SIZE_P2); ++i)
        {
            memcpy(ip, cbuf, TREZOR_AES_BLOCK_SIZE);
            ctr_inc(cbuf);
            ip += TREZOR_AES_BLOCK_SIZE;
        }

        if(blen & (TREZOR_AES_BLOCK_SIZE - 1))
            memcpy(ip, cbuf, TREZOR_AES_BLOCK_SIZE), i++;
...
#endif
        if(aes_ecb_encrypt(buf, buf, i * TREZOR_AES_BLOCK_SIZE, ctx) != EXIT_SUCCESS)
            return EXIT_FAILURE;

        i = 0; ip = buf;
# ifdef FAST_BUFFER_OPERATIONS
        if(!ALIGN_OFFSET( ibuf, 4 ) && !ALIGN_OFFSET( obuf, 4 ) && !ALIGN_OFFSET( ip, 4 ))
            while(i + TREZOR_AES_BLOCK_SIZE <= blen)
            {
